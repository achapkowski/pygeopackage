...
def _copy_table_schema(con, table_source, table_name_dest):
    ...
    sql = """CREATE TABLE {dest} AS SELECT * FROM {source} WHERE 0""".format(source=table_source,
                                                                             dest=table_name_dest)
...
def _create_gpkg(name, path=None, overwrite=False):
    ...
    if len(result) < 2:
        ...
        _insert_values(con=con,
                      ...
                      fields=["srs_name", "srs_id", "organization", "organization_coordsys_id", "definition", "description"],
                      values=[
                          ["GCS_WGS_1984", 4326, "EPSG", "4326", """GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137.0,298.257223563]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]]""", "WGS 1984"],
...
def _create_table(con, name, fields=None):
    ...
    for k,v in fields.items():
        ...
        del k,v
    try:
        sql = """CREATE TABLE IF NOT EXISTS {tbl} ({fields})""".format(tbl=name, fields=",".join(txts))
        con.execute(sql)
        con.commit()
        _insert_values(con=con, tbl="gpkg_contents",
    ...
    **Argument**       **Description**
    ...
    for k,v in fields.items():
        ...
        if row[0].lower() != "text":
            ...
            txts.append(fld)
        del k,v
    sql = """CREATE TABLE IF NOT EXISTS {name} ({fields})""".format(name=name,
                                                                    fields=",".join(txts))
    try:
        con.execute(sql)
        con.commit()
    ...
    sql = """select srs_id from gpkg_spatial_ref_sys where srs_id = {wkid};""".format(wkid=wkid)
    ...
    if sr_check == 0:
        ...
        if len(res) > 0:
            res = res[0]
        _insert_values(con=con, tbl="gpkg_spatial_ref_sys",
                       fields=['srs_name', 'srs_id',
                               'organization', 'description',
                               'organization_coordsys_id', 'definition'],
                       values=[[res['NAME'], res['WKID'], "ESRI",
                                res['NAME'], res['WKID'], res['WKT']]])
    return True
